...
| Viewable draft pages (unlisted preview) | Today `/recipes/[slug]` **404s** drafts (`getPublicRecipe` returns null for non-public). Instead, **render** a draft when its URL is hit, with a clear **"Draft" badge**, so the owner can preview by URL (e.g. right after drafting via claude.ai) without signing into `/admin`. Publish stays gated to the admin screen. Drafts must stay **out of the index/tag listings + sitemap and be `noindex`** — reachable only if you know the slug. **Trade-off (explicit):** this changes drafts from *private* (hard 404) to *unlisted* (obscurity-based; slugs are title-derived, so somewhat guessable). Surface: open `getPublicRecipe` to drafts, add the badge + `noindex` metadata to the detail page, confirm cache/`generateStaticParams` (drafts render dynamically, `recipeTag` still busts on write), and decide whether `GET /api/recipes/{slug}` also opens for drafts or stays key-gated. | M | idea |
