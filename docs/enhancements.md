...
| Item | Notes | Size | Status |
|---|---|---|---|
| Mobile dashboard layout too cramped | **Done** — the dashboard `Row` now stacks on mobile (`flex-col`, `sm:flex-row`): the title/slug take full width and wrap (title `sm:truncate` only from `sm` up; slug `break-all`) instead of truncating off-screen, and the action buttons drop below and wrap. | S | **done** (branch `ux/admin-mobile-pagination`) |
| Dashboard pagination | **Done** — each section (Drafts / Published) paginates independently, server-side via search params (`?d` / `?p`), 20 rows/page, with a Prev/Next pager + range label. Added an optional `visibility` filter to `ListOptions` so each section queries its own count + page. Paging one section preserves the other's page. | S | **done** (branch `ux/admin-mobile-pagination`) |
...
| Unified owner site (no separate `/admin`) | **Scoped** → [`docs/phase-7-unified-owner-site.md`](phase-7-unified-owner-site.md). Collapse the main/`admin` split: the owner logs in on the main site and owner affordances (drafts in listings, edit/visibility/**Send to panel** on recipe pages) appear **inline**, gated by **server-side dynamic Suspense holes reading Clerk `auth()`** (Approach A). Invariant: the cached public shell stays auth-free and each owner hole fails closed to nothing, preserving today's Clerk-outage isolation. Needs a `clerkMiddleware` matcher broadening (main risk) — prototype three mechanics first. | L | **scoped** |
| Send to panel (kitchen panel sender) | **Done (in `/admin`)** — `sendToPanel` server action forwards a recipe's JSON-LD (+ `notes`, via `toJsonLd`) to the justmy.website kitchen panel with a server-only `JMW_PANEL_SERVICE_TOKEN`; `SendToPanelButton` on dashboard rows shows quiet "Sent ✓" or surfaces the panel's 400 errors. Passive, no DB write. Migrates onto the public detail page in phase 7. See [`docs/panel-recipes-sender-brief.md`](panel-recipes-sender-brief.md). | S | **done** |
...
| Better tag browsing/viewing | **Done** (wrapping-cloud direction; the `/tags` index page was the alternative, not built). Both chip strips (home `RecipeBrowser` + tag-page nav) now use a shared `TagCloud` that **wraps to multiple rows** instead of horizontal-scrolling, with a "Show all (N more) / Show less" toggle (collapses beyond `TAG_CLOUD_COLLAPSED`=12). Tags are ordered **most-used first** (`getPublicTags` now count-desc, alpha tiebreak) so popular tags land in the collapsed view. The tag page starts expanded when the current tag would sit in the hidden tail, so the active chip is always visible. Note: `TAG_CLOUD_COLLAPSED` lives in server-safe `lib/tags.ts`, not the `"use client"` component, so server components read the real value. | M | **done** (branch `ux/tag-cloud`) |
